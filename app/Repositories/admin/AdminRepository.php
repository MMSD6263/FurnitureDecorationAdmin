...
//		$request['ip']  =self::getIP();
    ...
    public static function adminEdit($requst)
    {
        ...
        if ($requst['id']) {
            ...
            $requst['password'] = substr(md5(md5($requst['passwd']) . 'to'), -20);
    ...
    public static function saveUserImage($filename)
    {
        $admin = new Admin();
        $admin->where(['id'=>session('admin.id')])->update(['picpath'=>$filename]);
    }
    ...
    public static  function changePwd($data)
    {
        $admin = new Admin();
        $updateData = [];
        $updateData['passwd'] = $data['pwd'];
        $updateData['password'] = substr(md5(md5($data['pwd']) . 'to'), -20);
        $bool = $admin->where(['id'=>session('admin.id')])->update($updateData);
        if($bool){
            return self::message(true,'修改成功！');
        }else{
            return self::message(false,'修改失败！');
        }
    }

    public static function getFrameColors()
    {
        $frame = new Frame();
        $list  = $frame->select(['id','css_setting'])->get()->toArray();
        foreach($list as &$value){
            $value['color'] = json_decode($value['css_setting'],true)['top_color'];
        }
        return $list;
    }

    public static function changeFrameColor($data)
    {
        
        $admin = new Admin();
        $updateData = [];
        $updateData['individuation_id'] = $data['color_id'];
        $bool = $admin->where(['id'=>session('admin.id')])->update($updateData);
        if($bool){
            return self::message(true,'修改成功！');
        }else{
    ...
    {
        $admin = new Admin();
        $ids   = $admin->select(['id'])->get()->toArray();
        return $ids;
    }
