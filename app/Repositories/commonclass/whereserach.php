...
class whereserach
{
    ...
    public static function whereName($table, $field, $content, $flag = false)
    {
    ...
    public static function wherecdate($table, $field, $start = '', $end = '')
    {
        if (!empty($start) && empty($end)) {
            $table = $table->where($field, '>=', intval(strtotime($start . "00:00:00")));
            return $table;
        }
        if (!empty($end) && empty($start)) {
            $table = $table->where($field, '<=', intval(strtotime($end . "23:59:59")));
            return $table;

        }
        if (!empty($start) && !empty($end)) {
            $table = $table->whereBetween($field, array(intval(strtotime($start . "00:00:00")), intval(strtotime($end . "23:59:59"))));
            return $table;
        }
    }
     ...
     * @return mixed
     */
    public static function wherecdatems($table, $field, $start = '', $end = '')
    {
        if (!empty($start) && empty($end)) {
            $table = $table->where($field, '>=', intval(strtotime($start)) * 1000) ;
            return $table;
        }

        if (!empty($end) && empty($start)) {
            $table = $table->where($field, '<=', intval(strtotime($end )) * 1000);
            return $table;
        }

        if (!empty($start) && !empty($end)) {
            $table = $table->whereBetween($field, array(intval(strtotime($start )) * 1000, intval(strtotime($end)) * 1000));
            return $table;
        }
    }


    public static function wheredate($table, $field, $start = '', $end = '')
    {
        if (!empty($start) && empty($end)) {
            $table = $table->where($field, '>=', $start);
            return $table;
        }
        if (!empty($end) && empty($start)) {
            $table = $table->where($field, '<=',$end);
            return $table;

        }
        if (!empty($start) && !empty($end)) {
            $table = $table->whereBetween($field, array($start,$end));
            return $table;
        }
    }
